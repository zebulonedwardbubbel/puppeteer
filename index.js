...
(async function createSheets() {
    ...
    const dataLength = returnedData.length;
    
    const wb = XLSX.utils.book_new();
    // works with single sheet
    // for (let i = 0; i < dataLength; i++) {
    //     let sheetName = `Sheeet${i}`;
    //     let sheet = XLSX.utils.json_to_sheet(returnedData[i]);
    //     XLSX.utils.book_append_sheet(wb, sheet, sheetName);
    // }
    const sheetName = `Sheeet`;
    const sheet = XLSX.utils.json_to_sheet(returnedData[0]);
    for (let i = 1; i < dataLength; i++) {
        XLSX.utils.sheet_add_json(sheet, returnedData[i], {skipHeader: true, origin: `A${returnedData[i - 1].length}`});
        console.log(i);
    }
    XLSX.utils.book_append_sheet(wb, sheet, sheetName);
